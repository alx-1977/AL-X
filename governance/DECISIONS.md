...
**Economics.** Luna at $0.20 uncached input, $0.02 cached, $1.20 output, $0.25 cache write per million; 32,000-token input and output bounds; worst-case reservation $0.0528 per autonomous Core call; `AUTONOMOUS_COGNITION_DAILY_BUDGET_USD=0.5405`, recorded as approximately R10/day at the design assumption of R18.5/USD. Spend is reserved before dispatch and reconciled against measured usage; missing usage retains the conservative reservation; an unpriced or unknown model fails closed; the budget persists across restart and never increases itself.

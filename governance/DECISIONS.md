...
- **Amendment approval — 2026-09-08.** Friedl explicitly approved the provider-shaped bill-budget amendment in the Codex conversation for `feat/claude-subscription-core`: his instruction to commit the complete feature state expressly included “bill-budget/recovery/checkpoint fixes” and “D-018 governance update”. He subsequently authorised this attribution while addressing the review of `dbad985ed6c7b3c558216426e0f2342fb8a3697e`.
- **The execution budget is provider-shaped, and it is not an authority.** Bill processing carries a ceiling on Core reasoning calls, which stops a task that has begun looping. It counts calls, so what it means depends on how many calls the configured reasoner spends on one useful turn. Metered API providers answer a bill turn in one or two calls and retain the existing strict budget: four calls, plus a bounded recovery allowance of two. `claude_subscription` reasons in more, smaller steps — an ordinary turn takes two calls and a real bill took four — so the same number stopped work that was progressing normally, and it uses a larger bounded budget of twenty-four calls with the same recovery allowance. An unrecognised provider receives the strict budget rather than the larger one.

  This changes no authority. The same capabilities, the same standing scopes, the same evidence rules and the same read-back verification apply at either ceiling; a higher ceiling permits more reasoning about one bill, never a different write. Recovery stays bounded and is not reissued per call, so a stopped task cannot reason indefinitely by re-entering it, and background continuity turns cannot consume the recovery allowance reserved for the person waiting on the conversation. The larger number exists to fit a provider's call shape, not to tolerate an indefinite loop: twenty-four calls on one bill is still unmistakably a loop, and it is still stopped.
...
branch breaks those lines, `bash <(git show main:scripts/alx) restart` needs
nothing but committed `main`.

...
class NoSemanticRankingTests(StoreTestCase):
    ...
    def test_the_store_never_reads_a_conversation(self) -> None:
        """Law 1, held at the module: goal listing cannot see what was said."""
        source = (REPOSITORY_ROOT / "src/alx/goals/store.py").read_text()
        for forbidden in ("turns", "ConversationSnapshot", "content ", "message"):
            with self.subTest(token=forbidden):
                self.assertNotIn(f"def list_unfinished{forbidden}", source)
        # The listing takes identifiers and a cap, and nothing resembling text.
        self.assertIn(

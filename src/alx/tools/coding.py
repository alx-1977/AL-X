...
DEFINITION = CapabilityDefinition(
    ...
    "name. A reviewer provider that cannot begin, such as an exhausted "
    "subscription, missing login or CLI, or no response, is not retried: the "
    "failure carries provider_unavailable true and its reason_code, the diff "
    "is preserved, and resuming the job repeats only the review. AL/X may supply only resume_job_id for a failed or cancelled call "

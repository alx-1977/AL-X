...
class SandboxRetention:
    ...
    async def run(self, interval_seconds: float = _SWEEP_INTERVAL_SECONDS) -> None:
        ...
        Retention was event-driven: at composition, and before each experiment.
        A runtime that is up but idle therefore never swept again, so the bytes
        of the last session outlived their TTL until somebody ran another
        experiment or restarted the process. D-027 sets a deadline in hours,
        and a deadline that only arrives when the next thing happens is not
        one.

        Low-frequency deliberately. The TTL is a day; sweeping every few
        minutes would be a busy loop over the filesystem to enforce a bound
        that moves once. Failures are swallowed for the same reason the sweep
        itself isolates them: a runtime must not stop because a directory
        ...
        while True:
            ...
            try:
                ...
                raise
            except Exception as error:  # noqa: BLE001 - a sweep is not the run
                LOGGER.warning(
                    "Periodic sandbox retention failed: %s", type(error).__name__
                )

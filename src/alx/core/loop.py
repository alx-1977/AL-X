...
class CoreAgent:
    ...
    def process(self, conversation: ConversationSnapshot, retention_until: datetime,
        ...
        for step_index in range(step_budget):
            ...
            if decision.call is None:
                # Checked before anything is committed or delivered. A response
                # claiming an action that never ran must not reach Friedl, and
                # must not leave memories or history behind describing a world
                # that did not change.
                #
                # This is the only gate for both delivery sites. A response
                # that later reaches the memory-conflict path has already
                # passed here, so that path cannot become a second, unchecked
                # route to saying something happened.
                claim_error = self._claimed_completion_error(
                    decision,
                    None if snapshot is None else snapshot.state,
                    transient_attempts,
                )
                if claim_error is not None:
                    LOGGER.info("Response claim rejected: %s", claim_error)
                    # She is told which claim failed and why, so she can issue
                    # the call she believed she had already made or describe
                    # the state truthfully. Nothing is dispatched on her
                    # behalf: an unexecuted claim is a reason to reason again,
                    # never an instruction to make the sentence true.
                    if self._already_refused(
                        refused_calls, claim_error, "response"
                    ):
                        return CoreOutcome(
                            CoreState.ERROR, snapshot, reason=claim_error,
                        )
                    refused_calls = (*refused_calls, {
                        "call_id": None,
                        "capability_id": None,
                        "reason": claim_error,
                        "subject": "response",
                        "claimed_completed_actions": list(
                            decision.claimed_completed_actions
                        ),
                    })
                    continue
                try:
    ...
    def _claimed_completion_error(
        self,
        decision: AgentDecision,
        state: GoalState | None,
        transient_attempts: tuple[CapabilityAttempt, ...] = (),
    ) -> str | None:
        """Why this response may not claim the actions it claims, if it may not.

        On 2026-09-10 the Core twice told Friedl mail was deleted when no
        delete had run. Nothing had gone wrong mechanically: the response was
        simply never checked against what had executed. Evidence, memories and
        calls were all grounded; the words that actually reach him were the one
        thing nothing verified.

        Whether a sentence claims completion is meaning and stays hers, so she
        names the call_ids the response reports as done. Whether a named call
        actually succeeded has one objectively correct answer, so it is checked
        here rather than trusted.

        The outcomes come from _mutation_outcomes, which reads only what the
        runtime recorded. Both directions are checked: a completion she names
        must have succeeded, and a call she describes as unfinished must not
        have succeeded, because understating what happened outside AL/X is its
        own untruth.

        This verifies what the response does say. It does not require the
        response to mention every mutation the turn recorded: that obligation
        refused turns whose Core simply omitted a declaration, and it broke
        six existing flows including the approved mail send.
        """
        outcomes = self._mutation_outcomes(state, transient_attempts)
        attempted = {
            item.call.call_id
            for item in (
                *(() if state is None else state.attempts), *transient_attempts,
            )
            if item.call is not None
        }
        for call_id in decision.claimed_completed_actions:
            if call_id not in outcomes:
                # Distinguished so she can tell the two apart: a call that
                # never happened is a different mistake from one that ran but
                # changes nothing outside AL/X.
                if call_id in attempted:
                    return "claimed_action_changes_nothing_external"
                return "claimed_action_not_attempted"
            if outcomes[call_id] is not CapabilityResultState.SUCCEEDED:
                # Pending, refused, failed: the world did not change.
                return "claimed_action_did_not_succeed"
        # Every mutation this turn recorded must be accounted for. Reporting
        # two deletions while a third silently failed is the same defect in a
        # smaller shape, and the attempt record is the runtime's, not hers.
        # Whatever she does say about an action must match the record, in
        # both directions. Calling a success unfinished understates what
        # happened to the world, which is its own untruth.
        claimed = set(decision.claimed_completed_actions)
        for call_id in decision.unfinished_actions:
            if outcomes.get(call_id) is CapabilityResultState.SUCCEEDED:
                return "unfinished_action_actually_succeeded"
            if call_id in claimed:
                return "action_both_completed_and_unfinished"
        return None
    ...
    def _mutation_outcomes(
        ...
        state: GoalState | None,
        transient_attempts: tuple[CapabilityAttempt, ...] = (),
    ) -> dict[str, CapabilityResultState | None]:
        """What externally observable mutations this goal and turn recorded.

        The authoritative account of whether an external action happened, taken
        only from attempts the runtime itself appended when it dispatched. The
        Core never writes this: it cannot add an entry, change an outcome, or
        remove one by declining to mention it.

        That is what makes the guarantee structural rather than voluntary. A
        turn that dispatched nothing produces an empty map, so there is no
        successful mutation in existence for any statement to cite, and no
        wording can conjure one.

        None as a value means the attempt exists but did not reach a terminal
        executed result, which is still a fact about that call.
        """
        attempts = (
            *(() if state is None else state.attempts),
            # A none/attention_state call executes without a goal, so its
            # attempt lives only on the turn. It still happened.
            *transient_attempts,
        )
        outcomes: dict[str, CapabilityResultState | None] = {}
        for item in attempts:
            if item.call is None:
                continue
            definition = self._definition(item.call.capability_id)
            if definition is None or not definition.externally_observable_mutation:
                # Reading a message or running a search changes nothing
                # outside AL/X, so neither is a completed external action and
                # neither may be offered as proof that one occurred.
                continue
            if not CoreAgent._attempt_is_citable_evidence_source(item):
                outcomes[item.call.call_id] = None
                continue
            outcomes[item.call.call_id] = item.result.state
        return outcomes

...
| --- | --- | --- |
| 0 — One outcome. One production path. | Replacement-specific tests enumerate the production entry points, registrations, dispatchers, handlers, routes and callable sequences for the named outcome and prove exactly one remains. Mutation tests restore a superseded entry point and prove the suite fails. Beneath those, the `[[outcomes]]` manifest in `architecture/boundaries.toml` declares named production outcomes whose single entry point `scripts/check_architecture.py` enforces by AST inspection: the declared entry exists, is defined exactly once and in its declared module, no two outcomes share an entry, and no declared superseded fragment reappears in `src/alx`. This is a named-entry-point floor covering only declared outcomes. It is not a semantic proof that no duplicate behaviour exists, it does not follow reachability or dynamic dispatch, and a passing run does not by itself satisfy this law. | Name the production outcome, identify the authoritative path, list every searched superseded path and show its production code was deleted. Confirm any retained shared primitive cannot independently achieve the outcome. |
